...
export function unescapeJsonString(input) {
    // Step 1: Remove all backslashes outside of quotes
    input = input.replace(/("(?:\\.|[^"\\])*"|'(?:\\.|[^'\\])*')|\\/g, (match, quotedString) => {
        if (quotedString) return quotedString; // Keep quoted strings as-is
        return ''; // Remove unescaped backslashes outside quotes
    });

    // Step 2: Validate and preserve escaped special characters inside quotes
    input = input.replace(/("(?:\\.|[^"\\])*"|'(?:\\.|[^'\\])*')/g, (match) => {
        return match.replace(/\\(n|r|t|b|f|v|\\|\/|"|')/g, (escapeMatch) => escapeMatch); // Preserve valid escapes
    });

    // Step 3: Convert standalone backslashes inside quotes to double backslashes
    input = input.replace(/("(?:\\.|[^"\\])*"|'(?:\\.|[^'\\])*')/g, (match) => {
        return match.replace(/\\(?![nrtbfv\\\/"'])/g, '\\\\'); // Convert single backslashes to double unless part of a valid escape sequence
    });
